...
coverage-clover: ## Generate code coverage in Clover XML format
	"vendor/bin/tester" -s -p pcov --colors 1 -C --coverage ./var/tmp/coverage.xml --coverage-src ./src tests/cases

coverage-html: ## Generate code coverage in HTML format
	"vendor/bin/tester" -s -p pcov --colors 1 -C --coverage ./var/tmp/coverage.html --coverage-src ./src tests/cases
